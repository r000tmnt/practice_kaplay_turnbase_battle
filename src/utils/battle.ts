...
export const attack = async (unit: Unit, target: Unit, uIndex: number, tIndex: number) => {
    ...
    return { unit, uIndex, number: dmg, crit: rng <= crit, tIndex, attribute: tragetAttribute, action: "attck" }
}
...
export const useItem = async (unit: Unit, target: Unit, uIndex: number, tIndex: number, item: Item) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        wait(0.7, () => {
            ...
            if(number > 0) resolve({ unit, number, uIndex, crit: false, tIndex, attribute: tragetAttribute, action: "item" })                
        })        
    })
}
